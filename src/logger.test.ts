...
describe('logger', () => {
  describe('defaultLogger', () => {
    ...
    it('should log warnings with prefix', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('[opencode-provider] Test warning');
    });

    it('should log errors with prefix', () => {
      defaultLogger.error('Test error');
      expect(consoleErrorSpy).toHaveBeenCalledWith('[opencode-provider] Test error');
    });

    it('should log debug with prefix', () => {
      defaultLogger.debug?.('Test debug');
      expect(consoleDebugSpy).toHaveBeenCalledWith('[opencode-provider] Test debug');
    });
  });
  ...
  describe('createContextLogger', () => {
    it('should prefix messages with context', () => {
      ...
      expect(baseLogger.warn).toHaveBeenCalledWith('[TestContext] warning message');
      expect(baseLogger.error).toHaveBeenCalledWith('[TestContext] error message');
      expect(baseLogger.debug).toHaveBeenCalledWith('[TestContext] debug message');
    });
  ...
  describe('logUnsupportedFeature', () => {
    it('should log feature name', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith('Unsupported feature: custom tools');
    });
    ...
    it('should include details if provided', () => {
      ...
      logUnsupportedFeature(logger, 'image URLs', 'Only base64 data URLs are supported');
