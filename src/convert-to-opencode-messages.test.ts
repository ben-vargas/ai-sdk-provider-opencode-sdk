  ...
  describe('convertToOpencodeMessages', () => {
    ...
    describe('user messages', () => {
      ...
      it('should warn about remote URLs', () => {
        ...
        expect(result.parts).toHaveLength(0);
        expect(result.warnings.some((w) => w.includes('Remote URLs'))).toBe(true);
      });
      ...
      it('should convert Uint8Array to base64 data URL', () => {
        ...
        expect((result.parts[0] as { url: string }).url).toMatch(/^data:image\/png;base64,/);
      });
        ...
        const prompt: LanguageModelV2Prompt = [
          {
            role: 'user',
            content: [
              { type: 'unknown' as 'text', text: 'test' },
            ],
          },
        ];
    ...
    describe('assistant messages', () => {
      ...
      it('should convert tool calls as synthetic context', () => {
        ...
        expect((result.parts[0] as { text: string }).text).toContain('[Tool Call: search]');
      });
      ...
      it('should warn about file parts in assistant messages', () => {
        ...
        expect(result.warnings.some((w) => w.includes('File parts in assistant'))).toBe(true);
      });
    });
    ...
    describe('tool messages', () => {
      it('should convert tool results as synthetic context', () => {
        ...
        expect((result.parts[0] as { text: string }).text).toContain('[Tool Result: search]');
        expect(result.warnings.some((w) => w.includes('Tool results in prompts'))).toBe(true);
      });
    ...
    describe('JSON mode', () => {
      it('should add JSON mode instruction', () => {
        ...
        expect(result.parts).toHaveLength(2);
        expect((result.parts[1] as { text: string }).text).toContain('valid JSON');
      });
