...
async function main() {
  ...
  try {
    ...
    function createTimeoutController(
      ...
      (
        controller as AbortController & { clearTimeout: () => void }
    ...
    try {
      ...
      (
        controller4 as AbortController & { clearTimeout: () => void }
      ).clearTimeout();
      console.log("   Response:", text);
      console.log("   ✅ Completed within timeout\n");
    } catch (error: unknown) {
    ...
    console.log(
      '- User-initiated cancellations (e.g., "Stop generating" button)',
    );
