...
async function main() {
  ...
  try {
    ...
    console.log(
      "Generating a technical article with real-time partial updates...\n",
    );
    ...
    for await (const partial of partialObjectStream) {
      ...
      if (!firstPartialTime) {
        ...
        console.log(
          `[First partial received after ${firstPartialTime - startTime}ms]\n`,
        );
      }
      ...
      const sectionsCount =
        "sections" in partial && Array.isArray(partial.sections)
          ? partial.sections.length
          : 0;
      const tagsCount =
        "tags" in partial && Array.isArray(partial.tags)
          ? partial.tags.length
          : 0;
      const hasReadingTime =
        "readingTimeMinutes" in partial &&
        partial.readingTimeMinutes !== undefined;
    ...
    console.log(
      `Time to first partial: ${firstPartialTime ? firstPartialTime - startTime : 0}ms`,
    );
